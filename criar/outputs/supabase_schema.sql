...
-- ============================================================
-- 3.1  Check constraints adicionais em orders
-- ============================================================
alter table public.orders
...
alter table public.orders
  ...
  check (delivery_mode in ('Entrega no endereço', 'Retirada no local'));

alter table public.orders
  drop constraint if exists orders_notes_len_check;
alter table public.orders
  add constraint orders_notes_len_check
  check (char_length(notes) <= 500);

alter table public.orders
  drop constraint if exists orders_address_len_check;
alter table public.orders
  add constraint orders_address_len_check
  check (char_length(address) between 3 and 200);

-- ============================================================
-- 3.2  Bloquear escalação de role por usuário comum
-- ============================================================
...
-- ============================================================
-- 3.3  Audit log de ações críticas
-- ============================================================
...
begin
  if tg_op = 'UPDATE' then
    ...
    values (auth.uid(), 'orders.update', 'orders', new.id::text, to_jsonb(old), to_jsonb(new));
    return new;
  elsif tg_op = 'DELETE' then
    insert into public.audit_logs(actor_id, action, entity, entity_id, before_data, after_data)
    values (auth.uid(), 'orders.delete', 'orders', old.id::text, to_jsonb(old), null);
    return old;
  end if;
  return null;
end;
